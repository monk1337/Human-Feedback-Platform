from fastapi import Form, APIRouter, UploadFile, Response, Request, BackgroundTasks
...
from src.tasks.tasks import map_datasets_to_recorders
from src.utils.utils import remove_extra_quotes

router = APIRouter()

@router.post("/new")
async def create_project(file: UploadFile, request: Request, response: Response, background_tasks: BackgroundTasks):
    ...
    project = ProjectModel(project_name=remove_extra_quotes(project["projectName"]), admin_name=remove_extra_quotes(project["adminName"]), admin_mobile=remove_extra_quotes(project["adminMobile"]), recorder=recorder, reviewer=reviewer, record_point=project["recordPoints"], review_point=project["reviewPoints"])

    project_id = save_project(file, project)

    background_tasks.add_task(map_datasets_to_recorders, project_id)
